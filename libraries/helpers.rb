...
module SquidCookbook
  module Helpers
    ...
    def squid_load_url_acl(databag_name, acl_element)
      url_acl = []
      begin
        data_bag(databag_name).each do |bag|
          group = data_bag_item(databag_name, bag)
          ...
          group['urls'].each do |url|
            url_acl.push [group['id'], group.key?('element') ? group['element'] : acl_element, url]
          end
        end
      rescue
        Chef::Log.info "no '#{databag_name}' data bag"
    ...
    def squid_version_detected(package_name)
      command = %(#{package_name} -v | grep Version | sed 's/.*Version \\\(.\\..\\\).*/\\1/g' | tr -d '\n')
